...
def _plan_page_objects(
    ...
    *,
    ...
    arena_bytes: int,
) -> tuple[tuple[NativePageObject, ...], int]:
    ...
    try:
        ...
        raise NativeHybridRestoreError(
            f"flat page root identity cannot be verified: {error}"
        ) from error
    if (
        ...
        raise NativeHybridRestoreError("flat page root identity is not authenticated")
    ...
    if (
        ...
        raise NativeHybridRestoreError("flat page macro geometry is invalid")
    ...
    if expected_start != total:
        raise NativeHybridRestoreError("flat page objects do not cover the snapshot")
...
def _execute_page_object_restore(
    *,
    ...
    if hashlib.sha256(first_payload).hexdigest() != first.sha256:
        raise NativeHybridRestoreError(f"page object SHA-256 mismatch for {first.path}")
    ...
    with transaction:
        for index, page_object in enumerate(objects):
            arena_index = index % native.ARENA_COUNT
            arena = transaction.acquire_arena(arena_index)
            buffer = native.arena_memoryview(
                arena,
        ...
        if snapshot_digest.hexdigest() != manifest.get("snapshot_sha256"):
            raise NativeHybridRestoreError("flat page snapshot checksum mismatch")
        started = time.perf_counter()
        stats = transaction.finish()
        finish_ms = 1e3 * (time.perf_counter() - started)
