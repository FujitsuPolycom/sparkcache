...
class ManifestStoreTests(unittest.TestCase):
    ...
    def test_page_delta_chain_restores_intermediate_stage_boundaries(self) -> None:
        """A delta chain reconstructs any stored stage boundary byte-exactly.

        Agent sessions extend stored contexts turn over turn. The lookup path
        can offer an earlier chain boundary only if the restore path can
        materialize the state at that boundary without applying later stages.
        """
        from sparkcache.spark_context_cache_codec import (
            context_prefix_digest,
            pack_positions,
        ...
        tokens = tuple(range(1024))
        salt = "stage-boundary-restore-test"
        base_digest = context_prefix_digest(tokens, salt, token_count=256)
        second_digest = context_prefix_digest(tokens, salt, token_count=512)
        third_digest = context_prefix_digest(tokens, salt, token_count=768)
        base_snapshot = encode_page_snapshot(layout, (1,), {"page": b"A" * 1024})
        second_snapshot = encode_page_snapshot(
            layout, (2,), {"page": b"A" * 1024 + b"B" * 1024}
        )
        third_snapshot = encode_page_snapshot(
            layout, (3,), {"page": b"A" * 1024 + b"B" * 1024 + b"C" * 1024}
        )
        base_chunks = tuple(
            ContextChunk(
                index * 256,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            store.commit_page_extension(
                ...
                base_boundary_tokens=256,
                result_boundary_tokens=512,
                result_snapshot=second_snapshot,
            )
            third_lookup = store.lookup(identity, second_digest)
            self.assertTrue(third_lookup.is_hit, third_lookup.reason)
            prepared = store.restore_page_snapshot(
                third_lookup,
                layout=layout,
                result_block_counts=(2,),
                result_boundary_tokens=512,
            ...
            store.commit_page_extension(
                identity=identity,
                base_context_digest=second_digest,
                token_ids=tokens,
                identity_salt=salt,
                layout=layout,
                base_block_counts=(2,),
                ...
                result_boundary_tokens=768,
                result_snapshot=third_snapshot,
                verified_base_snapshot=prepared,
            )
            chain_lookup = store.lookup(identity, third_digest)
            self.assertTrue(chain_lookup.is_hit, chain_lookup.reason)
            self.assertEqual(len(chain_lookup._manifest["delta_stages"]), 2)  # type: ignore[index]
            # The chain head restores the final boundary...
            self.assertEqual(
                store.restore_page_snapshot(
                    chain_lookup,
                    layout=layout,
            ...
            # ...and the first stage boundary restores the same bytes the
            # original two-page publication produced, without applying the
            # later stage.
            self.assertEqual(
                store.restore_page_snapshot(
                    chain_lookup,
            ...
            # A boundary no stage produced is a clean format error.
            with self.assertRaisesRegex(
                cache_manifest.CacheFormatError, "no stage at the requested boundary"
            ):
                store.restore_page_snapshot(
                    chain_lookup,
                    layout=layout,
                    result_block_counts=(3,),
                    result_boundary_tokens=700,
                )
