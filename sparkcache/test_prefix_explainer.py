...
def test_readme_links_explorer_and_states_capabilities() -> None:
    ...
    required = (
        ...
        "Longest exact-prefix selection | Reuse the longest stored part of a prompt",
        "Sparse row-prefix aliases | Point to reusable earlier row boundaries",
        "Copy-on-write publication | Store only changed row tails or changed physical pages",
        "SparkCache CUDA restore | Move verified page data",
        "Shared bases and GPU prefixes | Read a common stored base once",
        "Interactive prefix explorer",
        "Status describes repository behavior",
        "## Deployment profiles",
    )
    for fragment in required:
