...
from sparkcache.persistent_context_cache.cache_manifest import CommitReceipt
...
def test_two_watermarks_write_all_chunks_before_manifest_commit() -> None:
    ...
    assert runtime.take_committed() == {"a" * 64: transaction.receipt}
    assert runtime.take_committed() == {}
...
def test_full_depth_two_ring_aborts_only_new_cache_without_blocking_owners() -> None:
    ...
    assert runtime.take_committed() == {
        "0" * 64: writer.transactions["req-0"].receipt,
        "1" * 64: writer.transactions["req-1"].receipt,
    }
    assert leases.active_leases == 0
    assert not ring.entries
...
def test_slow_writer_never_blocks_poll_or_publishes_early() -> None:
    ...
    assert runtime.take_committed() == {
        "a" * 64: writer.transactions["req"].receipt
    }
    assert not ring.entries
...
def test_internal_abort_with_pending_gather_returns_without_waiting() -> None:
    """DEFECTS.md D-5: a cache-internal abort must not synchronize fences.

    The gather fence stays pending across the abort; the source-block lease
    and the ring ticket remain owned by the aborted context and drain on a
    later poll once the fence completes.
    """
    ...
    assert offered.submitted_batches == 1
    assert leases.active_leases == 1
    # The fence is not ready; a regressing watermark aborts the publication.
    result = runtime.accept_completed_prefill(
        request_id="req",
        completed_tokens=128,
        block_table=block_table(64),
    ...
    assert result.aborted
    assert result.reason == "invalid_completed_watermark"
    assert writer.transactions["req"].aborted
    # Nothing was synchronized: the lease is held, the ticket is owned, and
    # the context stays alive for the poll path to drain.
    assert leases.active_leases == 1
    assert len(ring.entries) == 1
    assert runtime.active_contexts == 1
    assert runtime.needs_progress
    assert runtime.take_aborted() == {"req": "invalid_completed_watermark"}
    assert runtime.needs_progress
    ...
    assert not ring.entries
    assert runtime.active_contexts == 0
    assert not runtime.needs_progress
    assert runtime.counters["aborted_gathers_drained"] == 1


def test_internal_abort_releases_already_complete_gather_immediately() -> None:
    runtime, ring, writer, leases = make_runtime()
    begin(runtime, span_tokens=512)
    runtime.accept_completed_prefill(
        request_id="req",
    ...
    assert result.aborted
    # A complete fence needs no deferral: lease and ticket release at the
    # abort edge and the context retires.
    assert leases.active_leases == 0
    assert not ring.entries
    assert runtime.active_contexts == 0


def test_shutdown_drains_deferred_abort_gathers() -> None:
    runtime, ring, writer, leases = make_runtime()
    begin(runtime, span_tokens=512)
    runtime.accept_completed_prefill(
        request_id="req",
