...
class StreamingSnapshotRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        # Preserve the publisher's exact commit receipt through the terminal
        # handoff.  Bounded-capacity accounting needs the allocation upper
        # bound before the connector can decide whether this manifest is safe
        # to advertise.
        self._committed_manifests: dict[str, Any] = {}
    ...
    def needs_progress(self) -> bool:
        ...
        with self._lock:
            return (
                ...
                or bool(self._committed_manifests)
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                self._abort_context(
                    context, "invalid_block_table", wait_for_fences=False
                )
            ...
            try:
                ...
                # ownership ambiguity: no gather was submitted. Abandon
                # this optional cache publication instead of failing serving.
                self._abort_context(
                    context,
                    "invalid_completed_watermark",
                    wait_for_fences=False,
                )
                return RuntimeOffer(
                    aborted=True,
            ...
            if offer.aborted:
                self._abort_context(
                    context,
                    "planner_backpressure_or_mapping",
                    wait_for_fences=False,
                )
                return RuntimeOffer(
                    aborted=True,
    ...
    def take_committed(self) -> dict[str, Any]:
        """Drain manifests and receipts that crossed visibility successfully.

        The connector uses each preserved ``CommitReceipt`` to account for
        bounded NVMe capacity before advertising the digest in worker stats.
        A digest is inserted only after both the writer's manifest commit and
        the planner commit succeed; partial or aborted journals are therefore
        never surfaced as restorable entries.
        """
            ...
            for context in tuple(self._contexts.values()):
                for inflight in tuple(context.inflight.values()):
                    ...
                    if completion is None:
                        # A deferred-abort gather still owns its ticket.
                        # Bounded fence synchronization, then slot retire;
                        # a fence that cannot complete poisons rather than
                        # allowing teardown of memory the GPU may read.
                        try:
                            inflight.fence.synchronize()
                        ...
                        try:
                            ...
                            self._poison(error)
                        context.inflight.pop(
                            inflight.batch.batch_index, None
                        )
                        continue
                    try:
    ...
    def _submit_batch(
        ...
        *,
        ...
        if handle is None:
            ...
            self._abort_context(
                context, "lease_backpressure", wait_for_fences=False
            )
            return False
        try:
            ...
            self._abort_context(
                context, "physical_slot_mapping", wait_for_fences=False
            )
            return False
        ...
        if ticket is None:
            ...
            self._abort_context(
                context, "native_backpressure", wait_for_fences=False
            )
            return False
        fence = box["fence"]
        ...
        for inflight in tuple(
            ...
            if inflight.writer_completion is None:
                ...
                try:
                    ...
                    self._abort_context(
                        context, "writer_submit_failed", wait_for_fences=False
                    )
                    return completed
            ...
            try:
                ...
                self._abort_context(
                    context, "writer_result_failed", wait_for_fences=False
                )
                return completed
    ...
    def _commit_context(self, context: _Context, final_batch: SnapshotBatch) -> None:
        ...
        try:
            receipt = context.transaction.commit_manifest()
        except Exception:
            self.counters["manifest_commit_failed"] += 1
            self._abort_context(
                context, "manifest_commit_failed", wait_for_fences=False
            )
            return
        assert self._planner is not None
        try:
    ...
    def _abort_context(
        ...
        reason: str,
        *,
        wait_for_fences: bool = True,
    ) -> None:
        """Terminate a publication; the manifest can never become visible.

        ``wait_for_fences`` selects how in-flight gathers drain.

        ``True`` synchronizes every armed gather fence before returning, so
        the caller may hand the source blocks back for immediate reuse.
        Worker-side preemption requires this: vLLM overwrites preempted
        blocks as soon as ``handle_preemptions`` returns.

        ``False`` returns without waiting: armed leases stay held, in-flight
        tickets stay owned by the context, and the poll path (driven by the
        background progress worker) releases each one as its fence
        completes. Valid only when the source blocks are not reclaimed
        before the leases drain — the request still owns its block table
        during chunked prefill, and the delayed-free contract covers a
        finished request until ``take_finished`` reports it. Cache-internal
        aborts reachable from the serving thread use this mode so a slow
        gather never stalls inference.
        """
        if context.aborted:
            return
        ...
        assert self._leases is not None
        try:
            self._leases.abort_request(
                context.request_id, wait=wait_for_fences
            )
        except LeaseFenceError as error:
            self._poison(error)
        ...
        if wait_for_fences:
            ...
            try:
                ...
                self._poison(error)
            # Unclaimed tickets were retired by ring.abandon().  Claimed
            # tickets remain borrowed until their writer completions finish.
            context.inflight = {
                index: inflight
            ...
            # Fence-pending tickets must keep polling to completion before
            # their slots can be retired: an abandoned ticket's poll never
            # reports ready, which would leak its lease. Tickets whose fence
            # is already complete and that no writer borrowed are released
            # here (the ring accepts release of an unclaimed ready slot).
            remaining: dict[int, _Inflight] = {}
            for index, inflight in context.inflight.items():
                if inflight.writer_completion is not None:
                    remaining[index] = inflight
                    continue
                if inflight.fence.query():
                    assert self._ring is not None
                    try:
                        self._ring.release(inflight.ticket)
                    except Exception as error:
    ...
    def _drain_aborted(self, context: _Context) -> int:
        ...
        for inflight in tuple(context.inflight.values()):
            completion = inflight.writer_completion
            if completion is None:
                # Deferred-abort gather: the fence must complete before the
                # ticket's slot may be retired. The lease registry's poll
                # releases the source-block lease from the same fence.
                try:
                    fence_done = inflight.fence.query()
                except Exception as error:
                    self._poison(error)
                if not fence_done:
                    continue
                ...
                try:
                    ...
                    self._poison(error)
                context.inflight.pop(inflight.batch.batch_index, None)
                self.counters["aborted_gathers_drained"] += 1
                completed += 1
                continue
            try:
