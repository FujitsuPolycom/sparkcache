...
This module is deliberately independent of torch, CUDA and vLLM. Model-serving
integration supplies a small completion-fence adapter (normally wrapping
...
class BlockLeaseRegistry:
    ...
    def abort_request(self, request_id: str, *, wait: bool = True) -> int:
        """Release every lease owned by one request.
        ...
        ``wait`` selects what happens to armed leases. ``wait=True``
        synchronizes each armed fence before releasing its blocks — required
        whenever the caller's contract hands the blocks back for reuse the
        moment this method returns (worker-side preemption, shutdown).
        ``wait=False`` leaves armed leases in place for :meth:`poll` to
        release once their fences complete; the blocks stay leased, so
        :meth:`take_finished` continues to withhold the request. Valid only
        when nothing reclaims the blocks before the lease drains: the
        request still owns its block table, or the connector's delayed-free
        contract covers it.
        """
        ...
        if not wait:
            # Release armed leases whose fences already report complete;
            # only genuinely pending gathers stay for poll() to drain.
            done: list[_Lease] = []
            pending = 0
            for lease in armed:
                assert lease.fence is not None
                try:
                    if lease.fence.query():
                        done.append(lease)
                    else:
                        pending += 1
                except Exception as error:
            ...
            with self._lock:
                for lease in done:
                    ...
                    released += 1
            self.counters["deferred_abort_leases"] += pending
            return released
