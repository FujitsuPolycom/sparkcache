...
def test_worker_settings_enable_timing_only_with_explicit_one() -> None:
    ...
    assert not ModelServingStreamingSettings.from_connector(
        _connector(extra=base)
    ).timing_enabled
    assert ModelServingStreamingSettings.from_connector(
        _connector(
            extra={
                **base,
    ...
    with pytest.raises(RuntimeError, match="timing flag"):
        ModelServingStreamingSettings.from_connector(
            _connector(
                extra={
                    **base,
...
def test_worker_finishes_manifest_without_another_foreground_callback(
    ...
    class Runtime:
        ...
        def __init__(self) -> None:
            ...
            self.poll_calls = 0
            self._committed: dict[str, CommitReceipt] = {}
        ...
        def poll(self) -> int:
            ...
            self.active_contexts = 0
            self.needs_progress = True
            self._committed["9" * 64] = _commit_receipt()
            return 1
...
def test_lease_contract_is_verified_from_explicit_paths(tmp_path: Path) -> None:
    ...
    assert verify_model_serving_lease_contract(settings) == (source.resolve(),)
...
class _TerminalRuntime:
    ...
    def __init__(self) -> None:
        self.counters = {}
        self._committed = {"1" * 64: _commit_receipt(4096)}
...
def test_worker_advertises_only_post_manifest_committed_digests() -> None:
    ...
    def handoff(committed: dict[str, CommitReceipt]) -> set[str]:
        handed_off.update(committed)
        connector._held.update(committed)
        connector.counters["streaming_store_committed"] = len(committed)
        return set(committed)

    connector._handoff_streaming_commits = handoff
    adapter = WorkerStreamingSnapshotAdapter(
        connector,
        settings=ModelServingStreamingSettings(
            native_library_path=Path("/opt/spark/lib/libspcc_snapshot.so"),
    ...
    assert handed_off == {"1" * 64: _commit_receipt(4096)}
...
def test_finished_scheduler_generation_retains_context_until_late_commit(
    ...
    runtime._committed = {digest: _commit_receipt(65536)}
    with caplog.at_level(
        logging.INFO,
        logger="vllm.spark_context_cache.streaming",
    ):
