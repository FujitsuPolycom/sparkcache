...
import sparkcache.spark_context_cache_store as connector_store
...
def test_runtime_and_manifest_writer_compose_with_runtime_owned_ring(
    ...
    committed = runtime.take_committed()
    assert set(committed) == {digest}
    assert committed[digest].committed_tokens == 512
    assert committed[digest].allocated_bytes_upper_bound > 0
...
def test_runtime_publisher_uses_connector_store_abi_without_class_identity_leak(
    ...
    """Source-mounted serving loads the store through the flat staged-module shim.
    ...
    committed = runtime.take_committed()
    assert set(committed) == {digest}
    assert committed[digest].committed_tokens == 256
    assert committed[digest].allocated_bytes_upper_bound > 0
    runtime.shutdown()
    writer.shutdown()
