...
def test_canonical_cuda_symbols_alias_legacy_python_api() -> None:
    assert (
    ...
    assert cuda_page.CudaPageRestoreError is legacy_page.NativeHybridRestoreError
    assert (
        cuda_page.execute_cuda_page_placement
        is legacy_page.execute_native_hybrid_placement
    )
    assert (
        cuda_page.execute_cuda_direct_restore
