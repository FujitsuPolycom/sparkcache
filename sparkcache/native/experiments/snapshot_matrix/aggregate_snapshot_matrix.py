...
RAW_FIELD_MAP = {
    ...
    "standalone.glm52_full_size_payload_bytes": "common(geometry.used_bytes)",
    "standalone.full_size_byte_checks_per_rank": "cpu_readback.checks",
...
def _validate_probe(record: Mapping[str, Any]) -> tuple[str, int, int]:
    ...
    if config["slot_bytes"] not in (32 * MIB, 64 * MIB):
        raise AggregationError(
            "GLM-5.2 full-size fixture slot must be exactly 32 or 64 MiB"
        )
...
def _aggregate_cell(
    ...
    return {
        ...
        "ring": {
            ...
            # The producer exposes no portable fault-event counter.
            # Selection is based on measured first-touch/end-to-end latency.
            "managed_fault_events": 0,
        },
        "standalone": {
            "glm52_full_size_payload_bytes": GLM52_PAYLOAD_BYTES,
            "full_size_byte_checks_per_rank": [
