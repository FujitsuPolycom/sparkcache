...
Choose a SparkCache streaming-snapshot configuration for model-integrated
qualification:
...
Mapped depth 2 is the default because it reserves less memory. A third slot is
useful only if it
...
## Standalone probe versus model-integrated qualification

`spark_cache_snapshot_matrix_probe` covers both the compact 2 MiB
correctness fixture and the full-size `glm52` fixture with 101 sources
...
cache-off versus streaming-cache model-serving comparison required for
qualification.
...
lacks a result. Run the four-cell sequence only when more than one standalone
cell is indistinguishable; otherwise reload only the selected standalone
candidate.
...
- a standalone 2 MiB ABI gather and the pinned GLM-5.2 1,024-row full-size
  gather (exactly 32,743,424 bytes with the declared registered layout) in the
  configured 32 or 64 MiB slot;
- at least eight byte-exact full-size-payload CPU reads per rank;
...
- one model-serving 32K prefill/decode shadow at single-request concurrency
  (C1) and eight-request concurrency (C8);
- one large-context prefill long enough to emit at least 32 macro-batches.

The model-serving shadow must use the same model, KV budget, CUDA-graph capture
set, adaptive-MTP policy, prompt set, and serving activity as the cache-off
...
selection. Supply them for profile qualification.
  ...
  "cells": [
    {
      ...
      "standalone": {
        "glm52_full_size_payload_bytes": 32743424,
        "full_size_byte_checks_per_rank": [8, 8, 8, 8],
...
`"baseline": null, "live_candidate": null`. A model-integrated artifact
...
- a full-size payload of at least the pinned GLM-5.2 1,024-row size
...
- prefill, single-request decode (C1), and eight-request aggregate decode (C8)
  each at least 98% of cache-off;
...
4. Select managed memory only when its same-depth mapped cell also passes,
   ...
   qualification, its minimum serving ratio is no more than 0.5 percentage
   points worse.

Standalone selection is a reload shortlist, not model-serving qualification.
It can decisively reject incorrect geometry, fake depth, insufficient capacity,
managed-memory migration losses, unsafe shutdown, and excessive backpressure.
Only the selected candidate proceeds to the cache-off versus streaming-cache
model-serving comparison. If no cell passes, streaming snapshots remain
disabled.
...
the exact, warm, and total-consumption byte counts against the full GLM-5.2
payload. It rejects older records that do not carry these population counters.

Exit code `0` means at least one configuration is eligible for qualification.
Exit code `1` means the standalone matrix is complete but no cell passes. Exit
code `2` means the input is malformed or incomplete. Exit code `3` means a
standalone candidate was selected but model-serving evidence has not qualified
it.
