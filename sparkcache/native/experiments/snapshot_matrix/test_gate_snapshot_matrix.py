...
def _cell(mode: str, depth: int) -> dict:
    ...
    return {
        ...
        "standalone": {
            "glm52_full_size_payload_bytes": payload_bytes,
            "full_size_byte_checks_per_rank": byte_checks,
...
def test_conservative_passing_matrix_selects_mapped_depth_two() -> None:
...
def test_managed_cpu_path_rejects_standalone_cell() -> None:
    ...
    assert (
        "CPU readback did not consume every checked GLM-5.2 full-size payload"
        in result["reasons"]
    )
...
def test_payload_below_exact_glm52_boundary_is_not_full_size_evidence() -> None:
    payload = _matrix()
    payload["baseline"] = None
    payload["live_candidate"] = None
    payload["cells"][0]["standalone"]["glm52_full_size_payload_bytes"] = (
        GLM52_FULL_SIZE_PAYLOAD_BYTES - 1
    )
    ...
    assert not result["standalone_passed"]
    assert (
        "GLM-5.2 full-size payload is below the pinned GLM-5.2 1024-row "
...
def test_exact_glm52_payload_boundary_passes_with_full_readback() -> None:
    ...
    standalone["glm52_full_size_payload_bytes"] = GLM52_FULL_SIZE_PAYLOAD_BYTES
    standalone["cpu_readback_bytes_per_rank"] = [
        GLM52_FULL_SIZE_PAYLOAD_BYTES * check_count
        for check_count in standalone["full_size_byte_checks_per_rank"]
    ]
