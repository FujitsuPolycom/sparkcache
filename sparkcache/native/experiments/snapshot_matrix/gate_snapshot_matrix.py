...
MIB = 1024 * 1024
GIB = 1024 * MIB
GLM52_FULL_SIZE_PAYLOAD_BYTES = 32_743_424
MAXIMUM_FULL_SIZE_SLOT_BYTES = 64 * MIB
...
def _evaluate_cell(
    ...
    *,
    ...
    glm52_full_size_payload_bytes = int(
        _number(standalone, "glm52_full_size_payload_bytes")
    )
    if not (
        GLM52_FULL_SIZE_PAYLOAD_BYTES
        <= glm52_full_size_payload_bytes
        <= MAXIMUM_FULL_SIZE_SLOT_BYTES
    ):
        standalone_reasons.append(
            "GLM-5.2 full-size payload is below the pinned GLM-5.2 1024-row "
            "payload or above 64 MiB"
        )
    if glm52_full_size_payload_bytes > slot_bytes:
        standalone_reasons.append("GLM-5.2 full-size payload exceeds slot capacity")
    checks = _integers(
        standalone, "full_size_byte_checks_per_rank", rank_count
    )
    if min(checks) < 8:
        standalone_reasons.append(
            "fewer than eight full-size-payload byte checks on a rank"
        )
    readback_bytes = _integers(
        standalone, "cpu_readback_bytes_per_rank", rank_count
    )
    if any(
        observed < glm52_full_size_payload_bytes * check_count
