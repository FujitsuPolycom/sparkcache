...
def _verify_required_symbols(
    ...
    if not isinstance(required_symbols, list) or any(
        ...
        raise ContractError(f"invalid required_symbols for {relative}")
    ...
    try:
        ...
        raise ContractError(f"cannot inspect symbols in {relative}: {error}") from error
    ...
    for symbol in required_symbols:
        ...
        if len(parts) != 2 or not all(parts):
            raise ContractError(f"invalid required symbol {symbol!r} for {relative}")
        class_name, member_name = parts
        class_node = classes.get(class_name)
        if class_node is None:
            raise ContractError(f"required symbol absent in {relative}: {symbol}")
        members: set[str] = set()
