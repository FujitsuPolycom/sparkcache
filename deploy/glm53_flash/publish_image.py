#!/usr/bin/env python3
"""Push an attested GLM-5.3 image and bind its registry digest to an SBOM."""
...
class PublishError(RuntimeError):
    """Raised when a registry publication cannot prove its inputs."""
...
def publish(
    *,
    ...
    raw_digests = run(
        ("docker", "image", "inspect", "--format", "{{json .RepoDigests}}", destination)
    )
    repo_digests = sorted(json.loads(raw_digests))
    repository = destination.rsplit(":", 1)[0]
    matching = [value for value in repo_digests if value.startswith(repository + "@sha256:")]
    if len(matching) != 1:
