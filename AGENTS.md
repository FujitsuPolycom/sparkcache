...
- Explain concepts before identifiers. Do not make internal codenames,
  ...
  only after describing what they mean and only when the reader must use
  them.
...
- Comments explain invariants, intent, and non-obvious constraints, never
  change history. TODOs must name the missing condition and removal
  criterion.
...
- Defects are tracked in `DEFECTS.md` with stable identifiers that are never
  reused; an entry is removed in the commit that fixes it and its regression
  test names the identifier.
- Commits carry the repository author identity only; no generated-by or
  co-author trailers.
